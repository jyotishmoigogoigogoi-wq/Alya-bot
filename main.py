...
async def auto_cleanup_old_messages(bot=None):
    try:
        pool = await get_db()
        ...
        async with pool.acquire() as conn:
            count_result = await conn.fetchrow(
                "SELECT COUNT(*) as count FROM messages WHERE ts < $1",
                cutoff_date
            )
            deleted_count = count_result['count'] if count_result else 0
            await conn.execute("DELETE FROM messages WHERE ts < $1", cutoff_date)

        logger.info(f"Auto cleanup: Deleted {deleted_count} messages older than {CLEANUP_DAYS} days")

        if bot and deleted_count > 0:
            try:
                await bot.send_message(
                    chat_id=OWNER_ID,
                    text=f"🧹 Auto Cleanup Done!
...
async def cleanup_scheduler(bot):
    while True:
        await asyncio.sleep(CLEANUP_INTERVAL_HOURS * 3600)
        await auto_cleanup_old_messages(bot)
...
"
    ...
    if data == "confirm_clear_all_data":
        if not await is_owner(u.id):
            await q.answer("Access denied! Only owner can do this.", show_alert=True)
            return
...
'done' likho band karne ke liye.")
        ...
        if user_text == "🗑️ Clear All Data":
            if not await is_owner(u.id):
                await msg.reply_text("❌ Only owner can clear all data!")
                return
            await msg.reply_text(
...
async def main():
    ...
    await auto_cleanup_old_messages(app.bot)

    # Start background cleanup scheduler
    asyncio.create_task(cleanup_scheduler(app.bot))
